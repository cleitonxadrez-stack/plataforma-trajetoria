...
export function TrajetoriaTimeline({ stats, years }: { stats: TlStats; years: TlYear[] }) {
  ...
  const view = nq
    ? years.map((y) => ({ ...y, items: y.items.filter((i) => (i.title + " " + (i.natureza ?? "")).toLowerCase().includes(nq)) })).filter((y) => y.items.length)
    : years;
  ...
  return (
    <div className="tl">
      ...
      <div className="tl-years">
        {view.length === 0 && <p className="tl-empty">Nenhum item encontrado para “{q}”.</p>}
        {view.map((y) => {
          const isOpen = open.has(y.year) || !!nq;
