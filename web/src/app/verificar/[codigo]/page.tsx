...
export default async function VerificarPage(props: { params: Promise<{ codigo: string }> }) {
  ...
  return (
    <main className="max-w-2xl mx-auto px-6 py-12">
      ...
      <h1 className="serif text-3xl text-[#0f2942] mb-3">
      ...
      <section className="card mb-6">
        <h2 className="serif text-lg text-[#0f2942] mb-2">Existência e integridade</h2>
        {view.ok ? (
  ...
  return (
    <dl className="grid grid-cols-1 sm:grid-cols-[160px_1fr] gap-y-2 text-sm">
      ...
      <Term k="Fingerprint (SHA-256)" v={
        view.fingerprint ? (
          <span className="font-mono text-[#0f2942]">{view.fingerprint}…</span>
        ) : (
          "—"
        )
      } />
...
function Term({ k, v }: { k: string; v: React.ReactNode }) {
  return (
    <>
      <dt className="text-[#0f2942]/70 uppercase tracking-widest text-xs">{k}</dt>
      <dd className="text-[#0f2942]">{v}</dd>
    </>
  );
}
...
function Disclaimer() {
  return (
    <aside className="card bg-[#f3e3cd] border-[#a15a13]/40">
      <p className="text-xs text-[#0f2942]/80 leading-relaxed">
...
function NotFoundPage({ code, reason }: { code: string; reason: string }) {
  return (
    <main className="max-w-2xl mx-auto px-6 py-12">
      ...
      <h1 className="serif text-3xl text-[#0f2942] mb-2">Código não encontrado</h1>
