  ...
  return (
    <main className="mx-auto max-w-3xl px-6 py-10">
      <header className="mb-8">
        ...
        <p className="text-sm text-[#0f2942]/70 mt-2">
      ...
      <section className="card mt-6 space-y-6">
        <div font-sans text-base>
          ...
          <p className="text-xs text-[#0f2942]/70 mb-4">
...
function MethodRadio(props: {
  ...
  return (
    <li className="border border-[#0f2942]/10 rounded p-3">
      <label className="flex items-start gap-3 cursor-pointer">
        <input type="radio" name="method" value={props.id} defaultChecked={props.selected} />
        <div>
          <p className="font-medium">{props.label}</p>
          <p className="text-xs text-[#0f2942]/70 mt-0.5">{props.meta}</p>
        </div>
      </label>
    </li>
  );
